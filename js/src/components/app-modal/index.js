...
const overflowStyleName = {
	 ...
	 */
	auto: false,
	visible: 'swf-app-modal__styled--overflow-visible',
};
...
const AppModal = ( {
	...
	const modalClassName = classnames(
		// sfw-admin-page is for scoping particular styles to components that are used by
		// a SFW admin page and are rendered via ReactDOM.createPortal.
		'sfw-admin-page',
		'sfw-app-modal',
	...
	return (
		<Modal className={ modalClassName } { ...rest }>
			{ children }
			{ buttons.length >= 1 && (
				<div className="sfw-app-modal__footer">{ buttons }</div>
			) }
		</Modal>
	);
};
